...
## ASKBROWS: 4.0 — Фаза 4: CRM (админка) (2026-07-14)
- Вход в CRM: email + пароль (bcrypt) + **2FA (TOTP)**; отдельная admin-cookie. Bootstrap-админ из env, ссылка `otpauth://` в логах старта.
- Модель `AdminUser` + миграция `0002_admin`.
- Admin API (13 роутов): CRUD услуг, недельный график + TimeOff, настройки мастера (приватный адрес/шаг слота/часы отмены), база клиентов (поиск, карточка с историей/визитами/суммой), записи за период, **ручная запись** (в т.ч. новый клиент), смена исхода (`completed`/`cancelled`/`no_show`).
- Админ-фронт на `/admin110220`: логин(2FA) + разделы Записи / Услуги / График / Клиенты / Настройки.
- Базовые стили (не финальный дизайн) для форм/таблиц/кнопок — чтобы удобно проверять.
